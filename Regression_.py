...
def corr_mat(X):
    ...
    plt.matshow(X.corr(),fignum=f.number,cmap=plt.get_cmap('Blues_r'))
...
X_pos_train.iloc[:, 0:9:1] = scaler_pos.fit_transform(X_pos_train.iloc[:, 0:9:1])
X_neg_train.iloc[:, 0:10:1] = scaler_neg.fit_transform(X_neg_train.iloc[:, 0:10:1])

X_pos_test.iloc[:, 0:9:1] = scaler_pos.transform(X_pos_test.iloc[:, 0:9:1])
X_neg_test.iloc[:, 0:10:1] = scaler_neg.transform(X_neg_test.iloc[:, 0:10:1])
...
histogram_(error_pos,color='darkred')
histogram_(error_neg, color='lightblue')
...
histogram_(error_pos,color = "darkblue")
...
histogram_(error_neg,color='purple')
...
error_pos = y_pred_pos-y_pos_test
histogram_(error_pos,color = "pink")
...
error_neg = y_pred_neg-y_neg_test
histogram_(error_neg,color ='turquoise')
...
error_pos = y_pred_pos-y_pos_test
histogram_(error_pos,color = "darkgreen")
...
error_neg = y_pred_neg-y_neg_test
histogram_(error_neg,"lightgreen")
...
error_neg = y_pred_neg-y_neg_test
histogram_(error_neg,color="black")
